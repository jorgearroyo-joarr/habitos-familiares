...
class Profile(Base):
    ...
    # Relationships
    day_logs = relationship(
        "DayLog", back_populates="profile", cascade="all, delete-orphan"
    )
    rewards = relationship(
        "WeekReward", back_populates="profile", cascade="all, delete-orphan"
    )
    month_rewards = relationship(
        "MonthReward", back_populates="profile", cascade="all, delete-orphan"
    )
    habit_templates = relationship(
        "HabitTemplate", back_populates="profile", cascade="all, delete-orphan"
    )
    reward_tiers = relationship(
        "RewardTier", back_populates="profile", cascade="all, delete-orphan"
    )
...
class HabitTemplate(Base):
    ...
    micro_habits = relationship(
        "MicroHabit",
        back_populates="habit_template",
        cascade="all, delete-orphan",
        order_by="MicroHabit.sort_order",
    )
...
class MicroHabit(Base):
    ...
    habit_template_id = Column(
        Integer, ForeignKey("habit_templates.id", ondelete="CASCADE"), nullable=False
    )
...
class DayLog(Base):
    ...
    habit_entries = relationship(
        "HabitEntry", back_populates="day_log", cascade="all, delete-orphan"
    )
...
class HabitEntry(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    day_log_id = Column(
        Integer, ForeignKey("day_logs.id", ondelete="CASCADE"), nullable=False
    )
